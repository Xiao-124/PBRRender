...
uniform sampler3D g_tex3DCurrentOrderScattering;
uniform sampler3D g_tex3DHighOrderOrderScattering;

layout(rgba16f, binding=0)uniform IMAGE_WRITEONLY image3D g_rwtex3DHighOrderSctr;
...
void main()
{
    ...
    float3 f3HighOrderSctr = LoadTex3D_1( g_tex3DHighOrderOrderScattering, int4(ThreadId, 0) )_SWIZZLE3 +
                             LoadTex3D_1( g_tex3DCurrentOrderScattering, int4(ThreadId, 0) )_SWIZZLE3;
    imageStore( g_rwtex3DHighOrderSctr, _ToIvec(ThreadId), _ExpandVector( float4(f3HighOrderSctr, 0.0)));
}