...
static glm::vec2 find_cusp(float a, float b) noexcept 
{
    // First, find the maximum saturation (saturation S = C/L)
    float const S_cusp = compute_max_saturation(a, b);

    // Convert to linear sRGB to find the first point where at least one of r,g or b >= 1:
    glm::vec3 const rgb_at_max = OkLab_to_sRGB({ 1.0f, S_cusp * a, S_cusp * b });
    float const L_cusp = std::cbrt(1.0f / glm::max(rgb_at_max));
...
static float find_gamut_intersection(float a, float b, float L1, float C1, float L0) noexcept 
{
    ...
    glm::vec2 const cusp = find_cusp(a, b);
...
inline glm::vec3 gamut_clip_adaptive_L0_0_5(glm::vec3 rgb,
    float alpha = 0.05f, float threshold = 0.03f) noexcept 
{

    if (all(lessThanEqual(rgb, glm::vec3{ 1.0f + threshold })) &&
        all(greaterThanEqual(rgb, glm::vec3{ -threshold }))) {
        return rgb;
    }

    glm::vec3 const lab = sRGB_to_OkLab(rgb);

    float const L = lab.x;
    float const eps = 0.00001f;
    float const C = std::max(eps, std::sqrt(lab.y * lab.y + lab.z * lab.z));
