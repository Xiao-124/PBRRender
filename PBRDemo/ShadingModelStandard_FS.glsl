...
vec3 sampleSunAreaLight(const vec3 _lightDirection) 
{
    if (directLight.sun.w >= 0.0) 
    {
        float LoR = dot(_lightDirection, shading_reflected);
        float d = directLight.sun.x;
        highp vec3 s = shading_reflected - LoR * _lightDirection;
        return LoR < d ?
                normalize(_lightDirection * d + normalize(s) * directLight.sun.y) : shading_reflected;
    }
