...
static glm::mat3 Rec2020_to_XYZ
{
...
static glm::mat3 XYZ_to_Rec2020
{
...
static glm::mat3 XYZ_to_CIECAT16
{
...
static glm::mat3 CIECAT16_to_XYZ
{
...
static glm::mat3 XYZ_to_OkLab_LMS{
...
static glm::mat3 OkLab_to_OkLab_LMS{
...
static glm::mat3 OkLab_LMS_to_sRGB{
...
static glm::mat3 sRGB_to_Rec2020 = XYZ_to_Rec2020 * sRGB_to_XYZ;

static glm::mat3 Rec2020_to_sRGB = XYZ_to_sRGB * Rec2020_to_XYZ;

static glm::mat3 sRGB_to_LMS_CAT16 = XYZ_to_CIECAT16 * sRGB_to_XYZ;

static glm::mat3 LMS_CAT16_to_sRGB = XYZ_to_sRGB * CIECAT16_to_XYZ;

static glm::mat3 Rec2020_to_LMS_CAT16 = XYZ_to_CIECAT16 * Rec2020_to_XYZ;

static glm::mat3 LMS_CAT16_to_Rec2020 = XYZ_to_Rec2020 * CIECAT16_to_XYZ;

static glm::mat3 Rec2020_to_AP0 = AP1_to_AP0 * XYZ_to_AP1 * Rec2020_to_XYZ;

static glm::mat3 AP1_to_Rec2020 = XYZ_to_Rec2020 * AP1_to_XYZ;

static glm::mat3 Rec2020_to_OkLab_LMS = XYZ_to_OkLab_LMS * Rec2020_to_XYZ;

static glm::mat3 OkLab_LMS_to_Rec2020 = XYZ_to_Rec2020 * OkLab_LMS_to_XYZ;

//------------------------------------------------------------------------------
// Constants
//------------------------------------------------------------------------------

// Standard CIE 1931 2бу illuminant D65, in xyY space
constexpr glm::vec3 ILLUMINANT_D65_xyY{ 0.31271f, 0.32902f, 1.0f };

// Standard CIE 1931 2бу illuminant D65, in LMS space (CIECAT16)
// Result of: XYZ_to_CIECAT16 * xyY_to_XYZ(ILLUMINANT_D65_xyY);
constexpr glm::vec3 ILLUMINANT_D65_LMS_CAT16{ 0.975533f, 1.016483f, 1.084837f };

// RGB to luminance coefficients for Rec.2020, from Rec2020_to_XYZ
constexpr glm::vec3 LUMINANCE_Rec2020{ 0.2627002f, 0.6779981f, 0.0593017f };

// RGB to luminance coefficients for ACEScg (AP1), from AP1_to_XYZ
constexpr glm::vec3 LUMINANCE_AP1{ 0.272229f, 0.674082f, 0.0536895f };

// RGB to luminance coefficients for Rec.709, from sRGB_to_XYZ
constexpr glm::vec3 LUMINANCE_Rec709{ 0.2126730f, 0.7151520f, 0.0721750f };

// RGB to luminance coefficients for Rec.709 with HK-like weighting
constexpr glm::vec3 LUMINANCE_HK_Rec709{ 0.13913043f, 0.73043478f, 0.13043478f };
...
{
    // See http://en.wikipedia.orgwiki/Standard_illuminant#Illuminant_series_D/
...
{
    return { glm::vec2(v.x, v.y) / std::max(v.x + v.y + v.z, 1e-5f), v.y };
}

inline glm::vec3 pow3(glm::vec3 x) noexcept
{
    return x * x * x;
}

inline glm::vec3 sRGB_to_OkLab(glm::vec3 x) noexcept 
{
    return OkLab_LMS_to_OkLab * cbrt(sRGB_to_OkLab_LMS * x);
}

inline glm::vec3 Rec2020_to_OkLab(glm::vec3 x) noexcept 
{
    return OkLab_LMS_to_OkLab * cbrt(Rec2020_to_OkLab_LMS * x);
}

inline glm::vec3 OkLab_to_sRGB(glm::vec3 x) noexcept 
{
    return OkLab_LMS_to_sRGB * pow3(OkLab_to_OkLab_LMS * x);
}

inline glm::vec3 OkLab_to_Rec2020(glm::vec3 x) noexcept
{
...
inline glm::vec3 ACEScct_to_linearAP1(glm::vec3 x) noexcept {
...
inline glm::vec3 linearAP1_to_ACEScct(glm::vec3 x) noexcept 
{
    for (size_t i = 0; i < 3; i++) 
    {
        x[i] = x[i] < 0.0078125f
...
inline glm::vec3 OETF_Linear(glm::vec3 x) noexcept 
{
    return x;
}

inline glm::vec3 OETF_sRGB(glm::vec3 x) noexcept 
{
...
inline glm::vec3 EOTF_sRGB(glm::vec3 x) noexcept 
{
    constexpr float a = 0.055f;
    constexpr float a1 = 1.055f;
...
inline glm::vec3 OETF_PQ(glm::vec3 x, float maxPqValue) 
{
    ...
    glm::vec3 g = pow(x, PQ_constant_N);
    glm::vec3 numerator = PQ_constant_C1 + PQ_constant_C2 * g;
    glm::vec3 denominator = 1.0f + PQ_constant_C3 * g;
    return pow(numerator / denominator, PQ_constant_M);
}

inline glm::vec3 EOTF_PQ(glm::vec3 x, float maxPqValue) 
{
    constexpr float PQ_constant_N = 2610.0f / 4096.0f / 4.0f;
    constexpr float PQ_constant_M = 2523.0f / 4096.0f * 128.0f;
    constexpr float PQ_constant_C1 = 3424.0f / 4096.0f;
    ...
    glm::vec3 g = pow(x, 1.0f / PQ_constant_M);
    glm::vec3 numerator = max(g - PQ_constant_C1, 0.0f);
    glm::vec3 denominator = PQ_constant_C2 - (PQ_constant_C3 * g);
    glm::vec3 linearColor = pow(numerator / denominator, 1.0f / PQ_constant_N);
...
glm::vec3 gamutMapping_sRGB(glm::vec3 rgb) noexcept;
